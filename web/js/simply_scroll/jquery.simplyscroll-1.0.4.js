...
$.simplyScroll.fn.extend({
	init: function() {
		...
		if (!this.o.horizontal) {
            ...
            {
    			this.itemMax +=  jQuery(this.$items[i]).width();
            }
...
//		this.posMax = this.$items.length * this.itemMax;
		...
		if (this.o.autoMode=='loop') {
            ...
            for(var i=0; i<this.$items.length; i++)
            {
                sumWidth += jQuery(this.$items.length[i]).width();
                addItems = i;
                if (sumWidth > this.clipMax ){
                  break;
                }
...
//			this.$list.css(this.dimension,this.posMax+(this.itemMax*addItems) +'px');
			this.$list.css(this.dimension,this.posMax+ (this.itemMax) +'px');
		}
...
this.funcMoveFaster = function() { self.moveFaster(); };
		this.funcMoveResume = function() { self.moveResume(); };


		if (this.auto) {
			if (this.o.pauseOnHover) {
				this.$clip.hover(this.funcMoveFaster,this.funcMoveResume);
			}
			this.moveForward();
		} else {
