...
(function($){
	...
	Construct.prototype = {
		...
		{	
            ...
            this.opts.css.each(function(elem,key){
                var cssKEY = elem;
                if (cssKEY.substring(0,1) != '/') self.cssUrl[key] = self.opts.pathCss + elem;
                else 
                self.cssUrl[key] = elem;
            });
		...
		*/	
		...
		getEditorDoc: function(html)
		{
			css = '';
            var self = this;
			this.cssUrl.each(function(elem, index){
				css += '<link media="all" type="text/css" href="' + self.cssUrl[index] + '" rel="stylesheet">';
			});
			...
			if (this.opts.visual)
			{
				if (this.doc)
				{
	    			try
		    		{
		    			...
		    			if (cmd == 'inserthtml' && jQuery.browser.msie) 
                            this.doc.selection.createRange().pasteHTML(param);
		    			else   			
						{
                            if (cmd == 'JustifyJustify') 
                                this.get_selection().focusNode.parentNode.align = "justify";
                            else
                            {
		...
		*/	
		...
		showImage: function()
		{
            currentSelector = new ImageSelector('imageUploadCallback', this);
            currentSelector.show( 'alert' );
		...
		*/				
        ...
        showSize: function()
        {
            ...
            var handler = function()
            {
                ...
                $('#redactor_span_text').val(text);                
            }.bind(this);

            this.modalInit(RLANG.link, this.opts.path + 'plugins/size.html', 400, 300, handler);
    
        },    
        insertSize: function()
        {
            var value = $('#redactor_span_size').val();
            var text = $('#redactor_span_text').val();
            if (value == '') return true;
            var span = '<span style="font-size:' + value + 'px;">' + text + '</span> ';
    
            if (span)
            {
                    RedactorActive.frame.get(0).contentWindow.focus();
                    RedactorActive.execCommand('inserthtml', span);
...
{ 
	...
	rs.push(/<(meta|link|.?o:|.?style|.?div|.?head|.?html|body|.?body|.?font|.?span|!\[)[^>]*?>/g); // Unnecessary tags
