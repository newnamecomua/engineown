    ...
    <table class="text">
        <tr>
            {*<th style="text-align:center;">
                <u>
                    <a href="javascript:void(0);" onclick="window.doSort('id','{if $sort.dir=='asc'}desc{else}asc{/if}'); return false;">
                        ID заказа{if $sort.field=='id'}<img src="{$fvConfig->get('dir_web_root')}img/{$sort.dir}_arrow.gif" width="11" height="13">{/if}
                    </a>
                </u>
            </th>
              *}
        ...
        <tr>
            ...
            <td class="mixed">{if $ex->isTarget()}Да{else}Нет{/if}</td>
            <td class="mixed">
                {$fvModule->getEdit($ex)}
