<ul>
{foreach from=$ListV item=ex name="depend"}
         <li>
            <span>
                <div class="item" >
                <input type="hidden" name="d[{$ex->getPk()}]" value="{$smarty.foreach.depend.iteration}" />
                    <em class="marker {if $ex->hasChild()}open{/if}"></em>{$ex->getLang('name')}                
            ...
            {if $ex->hasChild()}
                {show_module module=$module view="index" type="v" parent_id=$ex->getPk() offset=$offsetPlus} 
            {/if}
         </li>             
{/foreach}
</ul>