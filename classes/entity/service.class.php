...
class Service extends fvRoot
{
    ...
    function validateUrl($value)
    {
        ...
        $inst = ServiceManager::getInstance()->getOneByurl($value);
        if (ServiceManager::getInstance()->isRootInstance($inst) && $inst->getPk() != $this->getPk()) {
            $valid = false;
    ...
    {
        $dir = fvSite::$fvConfig->get('path.upload.service' . ($isWeb ? "_web" : ""));
        if(!$isWeb)
    ...
    public function saveImage()
    {
        ...
        $dstImage = fvSite::$fvConfig->get('path.upload.service') . $this->image;
    ...
    {
        $dstImage = fvSite::$fvConfig->get('path.upload.service') . $this->oldImage;         
    ...
    {
        return ServiceManager::getInstance()->getServiceTypeList($this->service_type); 
    }
